...
$report = [ordered]@{
  ...
  checks = [ordered]@{
    ...
    startupRegistryConfigured = Test-Contains 'Root: HKLM; Subkey: "Software\Microsoft\Windows\CurrentVersion\Run"; ValueType: string; ValueName: "{#MyAppName}"; ValueData: """{sys}\wscript.exe"" ""{app}\start-agent.vbs"" --minimized"; Flags: uninsdeletevalue; Tasks: autostart'
