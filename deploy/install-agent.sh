...
if [[ -n "${AGENT_BINARY}" ]]; then
  ...
  "${GO_PATH}" build -C "${AGENT_SOURCE_DIR}" -ldflags "-X main.BuildVersion=${VERSION} -X main.BuildChannel=${DSC_RELEASE_CHANNEL:-test}" -o "${INSTALL_DIR}/device-state-console-agent" .
fi
...
echo "Update: sudo ${INSTALL_DIR}/device-state-console-agent update"
